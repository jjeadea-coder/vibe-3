...
1. 뉴스 수집 화면에서 RSS 출처와 키워드를 확인한다.
2. 필요한 경우 키워드 또는 RSS 출처를 추가한다.
3. `뉴스 수집` 버튼을 눌러 수동 수집을 실행한다.
4. 신규 저장 건수, 중복 건수, 실패 출처 수를 확인한다.
5. 기사 목록에서 검색, 키워드 필터, 출처 필터, 북마크 필터를 사용한다.

### 9.2 RSS 출처 관리

- RSS URL은 `http://` 또는 `https://`로 시작해야 한다.
- 같은 RSS URL은 중복 등록할 수 없다.
- 잘못된 RSS 출처가 있어도 다른 출처의 수집은 계속 진행된다.
- 기본 출처는 Google News 공공행정 RSS이며, 운영 환경에 맞는 공공기관/언론사 RSS로 교체할 수 있다.

### 9.3 중복 제거와 요약

- 기사 URL을 기준으로 중복 저장을 방지한다.
- 요약은 RSS의 `description` 또는 `summary` 값을 정리해서 저장한다.
- 기사 전문 크롤링과 AI 요약은 현재 버전 범위에 포함하지 않는다.

### 9.4 장애 대응

- 수집 결과의 실패 출처 수가 1 이상이면 RSS URL 접근 가능 여부를 먼저 확인한다.
- 외부망 접근이 제한된 환경에서는 수집 버튼이 실패할 수 있다.
- 내부망 운영 시에는 허용된 RSS URL만 등록하고, 필요하면 OS 작업 스케줄러에서 `/api/news/collect`를 호출하도록 확장한다.
